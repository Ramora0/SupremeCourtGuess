#!/usr/bin/env python3
"""
Export every Supreme Court case as a cleaned conversation transcript.

Pipeline: (1) ConvoKit case -> raw transcript (speaker + text, justice votes, outcome);
         (2) Clean transcript (normalize text/speakers, remove fillers, optional merge).

Usage:
  python all_cases_to_convo_cleaned.py [output_dir]
  MAX_UTTERANCES=50000 python all_cases_to_convo_cleaned.py [output_dir]
  python all_cases_to_convo_cleaned.py --merge-same-speaker --no-normalize-speakers out/
"""
...
def _speaker_last_name(convo, speaker_id) -> str:
    """Return last name only for a speaker (splits on space, takes last token)."""
...
def process_case(convo, normalize_speakers: bool = True, merge_same_speaker: bool = False) -> str:
    """Produce cleaned transcript for one case: convo -> raw -> clean."""
...
def main() -> None:
    import argparse
    parser = argparse.ArgumentParser(description="Export Supreme Court cases as cleaned transcripts.")
    parser.add_argument(
        "output_dir",
        nargs="?",
    ...
    for i, convo_id in tqdm(enumerate(convo_ids, 1), total=len(convo_ids), desc="Processing cases"):
        ...
        try:
            text = process_case(convo, normalize_speakers=normalize_speakers, merge_same_speaker=merge_same_speaker)
